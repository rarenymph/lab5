...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            double scale = 0.015;  // Зададим базовый масштаб
            double size = scale * Math.min(getWidth(), getHeight()); // Зависимость от размера окна
            double halfSize = size / 2;
            double endSize = halfSize / 2; // Половина длины концов

            // Преобразование координат точки для рисования
            Point2D.Double center = xyToPoint(point[0], point[1]);

            // Устанавливаем цвет маркера в зависимости от условия
            canvas.setColor(temp ? Color.BLUE : Color.BLACK);
